...
def preprocess_data(tabformer_base_path):
    ...
    transformer_dir = os.path.join(tabformer_base_path, "transformers")
    ...
    # # Save ID transformer
    # id_transformer_path = os.path.join(transformer_dir, "id_transformer.pkl")
    # with open(id_transformer_path, 'wb') as f:
    #     pickle.dump(id_transformer, f)
    # print(f"✓ Saved id_transformer to: {id_transformer_path}")
    
    # # Save feature transformer
    # transformer_path = os.path.join(transformer_dir, "transformer.pkl")
    # with open(transformer_path, 'wb') as f:
    #     pickle.dump(transformer, f)
    # print(f"✓ Saved transformer to: {transformer_path}")

    #  # Save metadata
    # id_columns = list(id_transformer.get_feature_names_out(MERCHANT_AND_USER_COLS))
    # feature_columns = list(transformer.get_feature_names_out(predictor_columns))
    
    # metadata = {
    #     'id_columns': id_columns,
    #     'feature_columns': feature_columns,
    #     'total_features': len(id_columns) + len(feature_columns),
    #     'merchant_and_user_cols': MERCHANT_AND_USER_COLS,
    #     'predictor_columns': predictor_columns,
    #     'numerical_predictors': numerical_predictors,
    #     'nominal_predictors': nominal_predictors,
    #     'binary_encoded': columns_for_binary_encoding,
    #     'onehot_encoded': columns_for_one_hot_encoding
    # }
    
    # metadata_path = os.path.join(transformer_dir, "metadata.json")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Save transformers from training data"
    )
    parser.add_argument(
        "--data-path",
        type=str,
        required=True,
        help="Path to TabFormer data directory (e.g., /data/TabFormer)"
    )
    
    args = parser.parse_args()
    
    preprocess_data(tabformer_base_path=args.data_path)
